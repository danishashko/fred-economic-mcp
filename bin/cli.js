...
/**
 * Cross-platform launcher for the FRED MCP server.
 *
 * The server itself is written in Python (FastMCP). This wrapper lets the
 * package be started with `npx -y fred-mcp-server` (or a global install) by:
 ...
 *
 * The FRED_API_KEY environment variable (set in the MCP client config) is
 * inherited by the child process automatically.
 *
...
const SERVER = path.join(PKG_ROOT, "fred_mcp.py");
const REQUIREMENTS = path.join(PKG_ROOT, "requirements.txt");

function log(msg) {
  process.stderr.write(`[fred-mcp] ${msg}\n`);
}
...
function ensurePythonEnv() {
  const baseCache =
    process.env.FRED_MCP_HOME ||
    path.join(os.homedir() || os.tmpdir(), ".cache", "fred-mcp");
...
function main() {
  ...
  if (!process.env.FRED_API_KEY) {
    // Not fatal: the server returns a clear instruction if the key is missing,
    // but warn early so users notice in their client logs.
    log(
      "Warning: FRED_API_KEY is not set. Get a free key at " +
        "https://fredaccount.stlouisfed.org/apikeys and add it to your MCP client config."
    );
  }
